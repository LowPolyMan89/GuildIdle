...
namespace GuildIdle.Editor.ConfigDownloader
{
    public sealed class BuildingsConfigsParser : IConfigPipelineParser
    {
        ...
        private const string SettlementStageBuildingsSheet = "SettlementStageBuildings";
        ...
        private static readonly string[] SettlementStageBuildingsRequiredColumns =
        {
            "stage_id",
            "building_id",
            "enabled",
        ...
        public ConfigPipelineReport BuildRuntimeJson(ConfigSourceSettings source, out string runtimeJson)
        {
            ...
            context.ValidateSettlementStageMembership();
        ...
        private sealed class BuildingsConfigContext
        {
            ...
            private readonly List<ConfigSheetDataRow> _settlementStageBuildings = new List<ConfigSheetDataRow>();
            ...
            public void ValidateSettlementStageMembership()
            {
                ValidateSettlementStageBuildingsTable();
            ...
            public Dictionary<string, List<Dictionary<string, object>>> BuildRuntimeArrays()
            {
                return new Dictionary<string, List<Dictionary<string, object>>>(StringComparer.Ordinal)
                {
                    ...
                    ["settlementStageBuildings"] = BuildSettlementStageBuildings(),
            ...
            private List<Dictionary<string, object>> BuildSettlementStageBuildings()
            {
                var rows = new List<Dictionary<string, object>>();
                foreach (var row in _settlementStageBuildings)
                {
                    rows.Add(new Dictionary<string, object>(StringComparer.Ordinal)
                    {
            ...
            private void ValidateSettlementStageBuildingsTable()
            {
                if (!_tables.TryGetValue(SettlementStageBuildingsSheet, out var table))
                {
                    AddIssue(SettlementStageBuildingsSheet, 0, string.Empty, string.Empty, "Required sheet is missing.");
                    return;
                }

                ValidateRequiredColumns(table, SettlementStageBuildingsRequiredColumns);
                if (!HasRequiredColumns(table, SettlementStageBuildingsRequiredColumns))
                    return;
                ...
                foreach (var row in table.DataRows)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(buildingId) && !_buildings.ContainsKey(buildingId))
                        AddIssue(SettlementStageBuildingsSheet, row.RowNumber, "building_id", buildingId, "building_id does not exist in Index.building_id.");

                    if (!string.IsNullOrWhiteSpace(stageId) && !string.IsNullOrWhiteSpace(buildingId))
                    {
                        var key = $"{stageId}\n{buildingId}";
                        if (seen.TryGetValue(key, out var firstRow))
                            AddIssue(SettlementStageBuildingsSheet, row.RowNumber, "building_id", buildingId, $"Duplicate stage_id + building_id; first declared at row {firstRow}.");
                        else
                            seen[key] = row.RowNumber;
                    }

                    if (hasEnabled && enabled)
                        _settlementStageBuildings.Add(row);
                }
            }
            ...
            {
                return string.Equals(sheetName, SettlementStageBuildingsSheet, StringComparison.OrdinalIgnoreCase) ||
