...
namespace GuildIdle.Editor.ConfigDownloader
{
    public sealed class ConfigCrossConfigValidatorTests
    {
        ...
        {
            ConfigSourceSettingsCollection BuildCollection(string targetId)
            {
                var quests = Download(
                    Sheet("QuestSteps",
                        Row("quest_id", "objective_type", "target_id"),
                        Row("quest_build_hut", "ActivityCompleted", targetId)));
                return Collection(
                    ...
                    Source("buildings_configs", "Buildings Configs", "quest-build-buildings.json", Download(
                        ...
                        Sheet("Hall", Row("building_id", "building_hall"), Row("level", "source_activity_id", "build_formula_id", "skill_id", "active_hero_limit"),
                            Row("0", "", "", "", "1"),
                            Row("1", "build_hall", "formula_build", "skill_construction", "1")))),
                    Source("quest_configs", "Quest Configs", "quest-build-quests.json", quests));
            }

            var validReport = ConfigCrossConfigValidator.Validate(BuildCollection("build_hall"));

            Assert.That(validReport.Success, Is.True, validReport.ToDisplayMessage());

            var invalidReport = ConfigCrossConfigValidator.Validate(BuildCollection("activity_missing"));
